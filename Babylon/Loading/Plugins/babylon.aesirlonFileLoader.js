...
(function () {
    ...
    var parseSpriteManager = function(parsedSpriteManager, scene, rootUrl) {
        ...
        if (parsedSpriteManager.instances) {
            for (var i = 0; i < parsedSpriteManager.instances.length; i++) {
                var parsedSprite = parsedSpriteManager.instances[i];
                var sprite = new BABYLON.Sprite(parsedSprite.name, spriteManager);
                sprite.position = BABYLON.Vector3.FromArray(parsedSprite.position);
                ...
                if (parsedSprite.animation) {
                    sprite.playNamedAnimation(parsedSprite.animation);
                }
            }
        }
    };
    ...
    BABYLON.SceneLoader.RegisterPlugin({
        ...
        load: function (scene, data, rootUrl) {
            ...
            if (parsedData.spriteManagers) {
                for (var index = 0; index < parsedData.spriteManagers.length; index++) {
                    var parsedSpriteManager = parsedData.spriteManagers[index];
                    parseSpriteManager(parsedSpriteManager, scene, rootUrl);
                }
            }
